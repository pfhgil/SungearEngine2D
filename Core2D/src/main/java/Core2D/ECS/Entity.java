    ...
    */
    ...
    public void setParentEntity(Entity parentEntity)
    {
        if(this.parentEntity != null) {
            ...
            if(parentEntity == null) {
                ...
                transform.setPosition(new Vector2f(transform.getPosition())
                        .mul(MatrixUtils.getScale(parentTransform.getGlobalModelMatrix()))
                        .add(MatrixUtils.getPosition(parentTransform.getGlobalModelMatrix())));
                transform.setScale(new Vector2f(transform.getScale()).mul(MatrixUtils.getScale(parentTransform.getGlobalModelMatrix())));
        ...
        if(parentEntity != null) {
            ...
            TransformComponent parentTransform = this.parentEntity.getComponent(TransformComponent.class);
            TransformComponent thisTransform = getComponent(TransformComponent.class);
            if(parentTransform == null || thisTransform == null) return;
            thisTransform.getTransform().setParentTransform(parentTransform.getTransform());
            /*
            Transform transform = getComponent(TransformComponent.class).getTransform();
            Transform parentTransform = this.parentEntity.getComponent(TransformComponent.class).getTransform();
            transform.setParentTransform(parentTransform);
            java.lang.System.out.println("attached parent transform: " + parentTransform);
            transform.setPosition(new Vector2f(transform.getPosition()).add(MatrixUtils.getPosition(parentTransform.getGlobalModelMatrix()).negate()));
            transform.setScale(new Vector2f(transform.getScale()).div(MatrixUtils.getScale(parentTransform.getGlobalModelMatrix())));
