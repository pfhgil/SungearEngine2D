...
public class TextureComponent extends Component implements NonDuplicated
{
    ...
    public void updateUV()
    {
        if(entity != null) {
            MeshComponent meshComponent = entity.getComponent(MeshComponent.class);
            meshComponent.getData()[2] = UV[0];
            meshComponent.getData()[3] = UV[1];

            meshComponent.getData()[6] = UV[2];
            meshComponent.getData()[7] = UV[3];

            meshComponent.getData()[10] = UV[4];
            meshComponent.getData()[11] = UV[5];

            meshComponent.getData()[14] = UV[6];
            meshComponent.getData()[15] = UV[7];

            if(meshComponent.getVertexArrayObject() != null) {
                VertexBuffer vbo = meshComponent.getVertexArrayObject().getVBOs().get(0);
                meshComponent.getVertexArrayObject().updateVBO(vbo, meshComponent.getData());
            }
        }
    }
}
